...
describe('termuxRuntime', () => {
  ...
  it('builds a bootstrap script that prepares daemon directories and start command', () => {
    ...
    expect(script).toContain("if (key === 'acp.probe-model-info') return { success: true, data: { modelInfo: getModelInfo(params.backend) } };");
    ...
    expect(script).toContain('useModel: modelLabel(currentModelId)');
