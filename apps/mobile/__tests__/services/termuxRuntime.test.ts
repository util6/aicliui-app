...
describe('termuxRuntime', () => {
  ...
  it('builds a bootstrap script that prepares daemon directories and start command', () => {
    ...
    expect(script).toContain("if (key === 'workspace.renameEntry') return await renameWorkspaceEntry(params);");
    ...
    expect(script).toContain('extractCodexPlanUpdate');
    expect(script).toContain("type === 'item.started' || type === 'item.completed'");
    expect(script).toContain("type === 'item.started' || type === 'item.updated' || type === 'item.completed'");
    ...
    expect(script).toContain('upsertCodexPlanMessage(conversationId, assistantMsgId, plan);');
    expect(script).toContain('function codexTodoListPlan(item)');
    expect(script).toContain('entries: items.map((todo) => ({');
    expect(script).toContain("status: todo.completed ? 'completed' : 'pending'");
    expect(script).toContain('function upsertCodexPlanMessage(conversationId, msgId, plan)');
    expect(script).toContain("message.type !== 'plan'");
    expect(script).toContain('message.content.sessionId === plan.sessionId');
    ...
    expect(script).toContain('onPlan: emitAssistantPlan');
